...
function assertConfiguration(configuration: PersistentMatcherConfiguration): void {
  ...
  if (configuration.solverQuotePolicy.maximumFeeBps !== 0n) {
    throw new Error("Persistent matcher requires an explicit zero-fee solver policy");
  }
}
...
export function matcherConfigurationHash(configuration: PersistentMatcherConfiguration): Hex32 {
  ...
  return keccak256Text([
    ...
    `solverCaps=${configuration.solverQuotePolicy.maximumFeeBps}:${configuration.solverQuotePolicy.maximumSlippageBps ?? 2_000n}`,
