import nativeZecUsdtMatcherManifestJson from "../../infra/matcher/native-zec-usdt.json" with { type: "json" };
...
export const NATIVE_ZEC_USDT_MATCHER_SCHEMA_VERSION = "1.0.0" as const;
export const NATIVE_ZEC_USDT_MATCHER_MANIFEST_TYPE = "native-zec-usdt-matcher-deployment" as const;
export const NATIVE_ZEC_USDT_MATCHER_SCHEMA_URL = "https://json-schema.org/draft/2020-12/schema" as const;

export const NATIVE_ZEC_NETWORK = ZCASH_MAINNET_NETWORK;
export const NATIVE_ZEC_ASSET = MAINNET_ZEC_ASSET;
export const NATIVE_USDT_ASSET = ETHEREUM_MAINNET_USDT_ASSET;
export const NATIVE_ZEC_USDT_MARKET_ID = "ZEC/USDT" as const;
export const NATIVE_ZEC_USDT_SETTLEMENT_PAIR = "ZEC-USDT" as const;
export const NATIVE_ZEC_USDT_PROTOCOL_VERSION = "transparent-htlc-v1" as const;
export const NATIVE_ZEC_USDT_ADAPTER_ID = adapterIdentifier(NATIVE_ZEC_USDT_PROTOCOL_VERSION);
...
export type NativeZecUsdtMatcherManifest = Readonly<{
  $schema: typeof NATIVE_ZEC_USDT_MATCHER_SCHEMA_URL;
  schemaVersion: typeof NATIVE_ZEC_USDT_MATCHER_SCHEMA_VERSION;
  manifestType: typeof NATIVE_ZEC_USDT_MATCHER_MANIFEST_TYPE;
  deployed: boolean;
  ...
  market: Readonly<{
    id: typeof NATIVE_ZEC_USDT_MARKET_ID;
    settlementPair: typeof NATIVE_ZEC_USDT_SETTLEMENT_PAIR;
    base: ManifestAsset;
    quote: ManifestAsset;
  }>;
  configurationHash: Hex32 | null;
  settlement: Readonly<{
    protocolVersion: typeof NATIVE_ZEC_USDT_PROTOCOL_VERSION;
    adapterId: Hex32;
...
export type NativeZecUsdtMatcherLimits = Readonly<{
...
export type NativeZecUsdtMatcherIdentity = Readonly<{
...
export type NativeZecUsdtMatcherDeploymentState = Readonly<{
  manifest: NativeZecUsdtMatcherManifest;
  deployed: boolean;
  submissionEnabled: boolean;
  enabled: boolean;
  configured: boolean;
  ...
  configurationHash: Hex32 | null;
  settlementProtocolVersion: typeof NATIVE_ZEC_USDT_PROTOCOL_VERSION;
  settlementAdapterId: Hex32;
  limits: NativeZecUsdtMatcherLimits;
  expectedMatcher: NativeZecUsdtMatcherIdentity | null;
}>;
...
function exactString(value: unknown, expected: string, label: string): string {
  ...
  if (actual !== expected) throw new Error(`${label} does not match the native ZEC/USDT manifest`);
  return actual;
}
...
function exactAsset(value: unknown, expected: ManifestAsset, label: string): ManifestAsset {
  ...
  if (asset.decimals !== expected.decimals) throw new Error(`${label} decimals do not match the native ZEC/USDT manifest`);
  chainIdentifier(expected.network);
  assetIdentifier(expected.asset);
  return expected;
}

function parseManifest(value: unknown): NativeZecUsdtMatcherManifest {
  const manifest = objectValue(value, "Native ZEC/USDT matcher manifest");
  assertExactKeys(manifest, [
    ...
    "settlement",
    "limits",
  ], "Native ZEC/USDT matcher manifest");

  exactString(manifest.$schema, NATIVE_ZEC_USDT_MATCHER_SCHEMA_URL, "Manifest $schema");
  exactString(manifest.schemaVersion, NATIVE_ZEC_USDT_MATCHER_SCHEMA_VERSION, "Manifest schema version");
  exactString(manifest.manifestType, NATIVE_ZEC_USDT_MATCHER_MANIFEST_TYPE, "Manifest type");
  ...
  exactString(market.id, NATIVE_ZEC_USDT_MARKET_ID, "Manifest market ID");
  exactString(market.settlementPair, NATIVE_ZEC_USDT_SETTLEMENT_PAIR, "Manifest settlement pair");
  ...
  exactString(settlement.protocolVersion, NATIVE_ZEC_USDT_PROTOCOL_VERSION, "Manifest settlement protocol");
  const adapterId = canonicalHex32(settlement.adapterId, "Manifest settlement adapter ID");
  if (adapterId !== NATIVE_ZEC_USDT_ADAPTER_ID) throw new Error("Manifest settlement adapter ID is not the configured protocol adapter");
  ...
  const canonical: NativeZecUsdtMatcherManifest = {
    $schema: NATIVE_ZEC_USDT_MATCHER_SCHEMA_URL,
    schemaVersion: NATIVE_ZEC_USDT_MATCHER_SCHEMA_VERSION,
    manifestType: NATIVE_ZEC_USDT_MATCHER_MANIFEST_TYPE,
    deployed,
    submissionEnabled,
    orderDomain: { name: "Phlebas Order Intent", version: "1" },
    evm: { network: ETHEREUM_MAINNET_NETWORK, chainId: 1, verifyingContract },
    market: {
      id: NATIVE_ZEC_USDT_MARKET_ID,
      settlementPair: NATIVE_ZEC_USDT_SETTLEMENT_PAIR,
      base,
      quote,
    },
    configurationHash,
    settlement: {
      protocolVersion: NATIVE_ZEC_USDT_PROTOCOL_VERSION,
      adapterId,
...
export function computeNativeZecUsdtMatcherConfigurationHash(verifyingContract: string): Hex32 {
  ...
  const source = parseManifest(nativeZecUsdtMatcherManifestJson);
...
export function parseNativeZecUsdtMatcherManifest(value: unknown): NativeZecUsdtMatcherDeploymentState {
  ...
  const limits: NativeZecUsdtMatcherLimits = {
