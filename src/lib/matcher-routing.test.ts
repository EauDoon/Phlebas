...
test("selects the lower zero-fee solver price over a complete order-book route", () => {
  const taker = order("taker", 0, 5_200n, 100_000_000n, 10n);
  const result = compare(taker, [order("maker", 1, 5_100n, 100_000_000n, 1n, 0)], [solver("solver", 1, 5_000n, 100_000_000n, 2n)]);
...
test("honors venue masks, quote expiry, zero-fee routing, and self-trade prevention", () => {
  ...
  assert.equal(compare(solverOnly, [], [expensiveFee]).selected, null);
...
test("skips fee-bearing and dust segments without failing the taker command", () => {
