...
function pad(input: Uint8Array): Uint8Array {
  ...
  for (let index = 0; index < 8; index += 1) {
    output[totalLength - 1 - index] = Number((bitLength >> BigInt(index * 8)) & 0xffn);
  }
  return output;
}
...
export function sha256(input: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < message.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + big1 + choice + ROUND_CONSTANTS[index] + words[index]) >>> 0;
...
export function sha256Hex(input: Uint8Array | string): Hex32 {
