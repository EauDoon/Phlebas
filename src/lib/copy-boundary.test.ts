...
test("landing and terminal banners stay simulation-only", async () => {
  ...
  assert.match(await readFile(join(root, "README.md"), "utf8"), /Native ZEC is not an ERC-20 token/);
...
test("architecture and accounting no longer list pZEC as the candidate ERC-20 form", async () => {
  ...
  assert.match(architecture, /A standard Uniswap v2 pool requires both assets in one contract state/);
...
test("remaining docs no longer list pZEC as the candidate ERC-20 form", async () => {
  ...
  const threat = await readFile(join(root, "docs/THREAT_MODEL.md"), "utf8");
  assert.match(threat, /gateway, reserve, mint, burn, and passive AMM sections below describe ADR 0001/);
  assert.match(threat, /replaces that target with native-ZEC atomic settlement/);
