...
- ADR 0010 is accepted (2026-09-05): option C's journal half is implemented (review-8 closed) — the matcher store appends journal records in place into one lifetime array, the `journal` getter documents `records` as a live reference, and a contract test pins it. Option B (accepted-order pruning with a versioned state root, per the acceptance-record amendment on the receipt-chain cross-binding) is the sequenced follow-up for review-1, which stays open until that change lands.
