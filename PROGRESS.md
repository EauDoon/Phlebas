...
- The float worst-price preview and `calculatePreviewNotional` are removed from `src/lib/order.ts` (review-5 closed): the ticket's market-order worst-price display is computed from `parseAtomicUnits` plus `worstPriceTicks`, the same primitives the submission path signs with, so there is one implementation of the worst-price primitive and the displayed worst price is the signed one.
- The matcher proxy's private 128-line copy of the strict JSON parser is removed (review-6 closed): `matcher-proxy.ts` imports `parseStrictJson` from `services/matcher/strict-json.ts` and defines none of the JSON grammar itself. The matcher image's source closure is unchanged because the import direction is proxy into services.
- ADR 0010 is accepted and fully implemented (2026-09-05): option C's journal half (review-8 closed) — the matcher store appends journal records in place into one lifetime array, the `journal` getter documents `records` as a live reference, and a contract test pins it — and option B's live accepted-order index (review-1 closed): terminal entries leave the index, the durable intake evidence stays complete and is checked body-free, `orderAccounts` prunes in lockstep, the state root is versioned through `MATCHER_STATE_ROOT_VERSION`, and the trade feed binds taker sides to the durable journal records. Pruned and unpruned artifacts cannot be silently compared; artifacts binding an older root fail loudly at open.
