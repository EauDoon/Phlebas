...
test("320px incident demo stays architecture-demonstration", async ({ page }) => {
  ...
  await expect(demo.getByRole("strong")).toHaveText("Historical observer-disagreement state.");
  await expect(demo.getByText(/retained atomic-swap observer is read-only/)).toBeVisible();
