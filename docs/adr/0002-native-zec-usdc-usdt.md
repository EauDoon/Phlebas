...
This ADR does not authorize live funds, a payable gateway, shielded ZEC, or live native-ZEC execution. Session encoding and undeployed contract sources may still use `pZEC` / `tpZEC` as the ERC-20 form and a historical `tUSDT0` faucet name. Product copy must not present those names as listed markets.
