...
* integer quote arithmetic and an explicit zero-fee policy;
...
A solver quote binds its matcher domain, signer, source and recipient accounts, exact networks and assets, side, capacity, minimum fill, fixed or curve price policy, slippage, fee field, nonce, expiry, and settlement protocol. The persistent matcher requires that field and its configured maximum to be zero. Capacity remains an advertised wallet-held amount. Consumption is linearized in the same durable sequence as matching.
...
* gross quote atoms, zero fee basis points, zero fee quote atoms, and an exact principal-only quote transfer;
...
The plan derives the gross quote from base atoms and execution price. A seller-side maker price rounds up and a buyer-side maker price rounds down. The exact principal transfer must remain between the seller's minimum and buyer's maximum signed quote bounds. Any positive protocol fee fails closed because the current exact-token escrow has no independently proven recipient split. Zero-value quote dust is rejected. An order salt is never reinterpreted as a hashlock digest.
