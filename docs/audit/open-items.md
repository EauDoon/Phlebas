...
| review-6 | `src/lib/matcher-proxy.ts` carries a private 128-line copy of the strict JSON parser that `services/matcher/strict-json.ts` exports. Both sit on an ingress path and both are what stands between an untrusted request body and the matcher. A differential fuzz over 400,000 mutated documents, 103,825 of which both accepted, found zero divergence today, so this is a hazard rather than a defect: the two agree, and nothing makes them keep agreeing. | Moving the parser into `src/lib` and having services import it adds a file to the matcher image's transitive source closure, which `services/infra.test.ts` pins against the Dockerfile's COPY list. That is a deliberate containment boundary and changing it belongs in a change that is about the boundary, not one that is about the parser. |
