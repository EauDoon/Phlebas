...
>>>>>>> origin/main
| review-9 | `tests/browser/phlebas.spec.ts` "landing Menu Markets opens the market section at 320px" asserts `#markets` takes focus after the mobile menu's Markets link is clicked. It fails on unmodified `origin/main`, twice out of two runs on a local machine, while passing in CI. The wiring looks correct: the section carries `tabIndex={-1}` and `followHash` in `landing-header.tsx` calls `target.focus({ preventScroll: true })`. The suspicion is that `closeMenu()`, which runs first, lets a dialog focus restoration overwrite the focus after `followHash` has set it, but that was not observed directly. | Reproduced and attributed, not diagnosed. Verified it is not a regression from the branch that found it, by running it against `origin/main`. Changing focus handling in `landing-header.tsx` on a suspicion, in a file the redesign work is actively touching, would be a guess dressed as a fix. It needs someone to observe what actually holds focus at the moment the assertion runs. |
