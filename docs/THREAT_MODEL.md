...
| Concurrent writers or double capacity consumption | Exclusive writer lock with a random ownership token, compare-before-delete release, and one serialized mutation queue | Distributed failover and cross-host fencing tokens are not implemented. A stale lock needs manual evidence-based recovery. |
