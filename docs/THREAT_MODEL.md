...
> Status as of 01-09-2026: design document and key-independent reference domain for a no-value simulation. The active EVM boundary is Ethereum Mainnet identity and unsigned action planning against an undeployed ConditionalLock manifest. Retired Sepolia sources remain isolated as historical contract-development records. No production contract, native transaction path, wallet execution, matching, routing, monitoring, or incident control is deployed or audited.
...
| Journal rollback or corruption | Restart loses, rewrites, or invents an accepted event | Bounded canonical disk replay checks exact initial identity and complete snapshot roots; durable writes precede acknowledgement and uncertain commits halt mutation. An independently retained checkpoint is still required to detect coherent rollback of every local file |
| Mixed Ethereum block reads | A number-tagged code or state read comes from a different block than the reviewed header | Pin code and state to the finalized block hash with canonicality required; reject changed block metadata and stale evidence before review |
| Malformed release evidence | An unknown gate, omitted browser result, or removed audit requirement produces a ready verdict | One shared evaluator requires every named gate exactly once; fixed canonical audit IDs prevent deletion or downgrade; malformed statuses and markers, failed or skipped checks cannot pass |
